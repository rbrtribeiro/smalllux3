...
static bool GetHitPointInformation(const luxrays::sdl::Scene *scene, luxrays::RandomGenerator *rndGen,
	...
	luxrays::sdl::TexMapInstance *tm = scene->objectTexMaps[currentMeshIndex];
	luxrays::sdl::BumpMapInstance *bm = scene->objectBumpMaps[currentMeshIndex];
	luxrays::sdl::NormalMapInstance *nm = scene->objectNormalMaps[currentMeshIndex];
	if (tm || bm || nm) {
...
class HitPoints {
public:
	HitPoints(luxrays::sdl::Scene *scene, luxrays::RandomGenerator *rndGen,
			luxrays::IntersectionDevice *device, luxrays::RayBuffer *rayBuffer,
			const unsigned int width, const unsigned int height, const unsigned int superSampling) {
		std::list<EyePath *> todoEyePaths;
		hitPoints = new std::vector<HitPoint>(width * height * superSampling * superSampling);
		unsigned int hitPointsIndex = 0;

		// Generate eye rays
		std::cerr << "Building eye paths rays with " << superSampling << "x" << superSampling << " super-sampling:" << std::endl;
		...
		const float photonRadius = ((ssize.x + ssize.y + ssize.z) / 3.f) / ((width * superSampling + height * superSampling) / 2.f) * 2.f;
...
public:
	...
	void ReHash() {
		...
		for (unsigned int i = 0; i < hitPointsCount; ++i) {
			...
			if (hp->type == SURFACE)
				maxPhotonRadius2 = luxrays::Max(maxPhotonRadius2, hp->photonRadius2);
		}
		...
		for (unsigned int i = 0; i < hitPointsCount; ++i) {
			...
			if (hp->type == SURFACE) {
				const float photonRadius = sqrtf(hp->photonRadius2);
		...
		if (hps) {
			...
			while (iter != hps->end()) {
				...
				if ((luxrays::Dot(hp->normal, shadeN) > 0.5f) &&
					...
					luxrays::Spectrum flux = photonFlux * hp->material->f(hp->wo, wi, shadeN) *
...
static void TracePhotonsThread(luxrays::RandomGenerator *rndGen, luxrays::IntersectionDevice *device,
	luxrays::RayBuffer *rayBuffer, const float alpha) {
		...
		for (unsigned int i = 0; i < rayBuffer->GetRayCount(); ++i) {
			...
			if (rayHit->Miss()) {
				...
				if (triMat->IsLightSource()) {
					...
					if (!specularBounce)
						hashGrid->AddFlux(alpha, hitPoint, shadeN, -ray->d, photonPath->flux);
...
int main(int argc, char *argv[]) {
	...
	try {
		...
		hitPoints = new HitPoints(scene, rndGen, device, rayBuffer, imgWidth, imgHeight, imgSuperSampling);
		...
		renderThread = new boost::thread(boost::bind(TracePhotonsThread, rndGen, device, rayBuffer, photonAlpha));
